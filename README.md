...
| 🔎 `/抖音` 搜索 | 在抖音综合页搜索关键词，按互动量与发布时间计算数据分后发送最高分结果。 | 指令白名单、互动阈值、搜索下拉次数、浏览器登录态。 |
...
| 命令 | 用途 | 注意事项 |
| --- | --- | --- |
| `/抖音 <关键词>` | 搜索抖音综合页，按互动量与发布时间计算数据分后发送最高分的可用结果。 | 不为全部候选调用 AI；结果受登录状态、互动阈值和平台页面结构影响。 |
...
| `config_version` | `1.2.9` | 配置结构版本。不要手动回退或随意改写；插件更新时会用它判断配置是否需要重建。 |
...
### 05 · `candidate_filter` — 候选互动门槛与发布时间
...
| `max_publish_age_days` | `0` | 候选距当天最多的自然日数；`0` 为不限制。填 `30` 时，综合搜索和推荐流只让近 30 天发布的视频进入候选库；页面日期未知也不会绕过该限制。 |

### 06 · `scoring` — 候选综合评分

最终 `share_score` 不再完全由模型自报，而是由互动、时效和 AI 深读内容判断合成。互动量使用对数曲线：达到参考值即为该项满分，之后不再无限抬高；抓取不到的互动数按该项 `0` 分处理，不会伪造热度。

| 字段 | 默认值 | 中文含义与修改建议 |
| --- | ---: | --- |
| `like_weight` / `comment_weight` / `collect_weight` / `share_weight` | `0.18` / `0.12` / `0.12` / `0.13` | 点赞、评论、收藏、转发的权重。 |
| `recency_weight` | `0.10` | 发布时间权重；当天发布得满分，随天数线性下降。 |
| `ai_weight` | `0.35` | AI 深读后的内容分享价值权重。模型仍会负责识别主题、趣味性、风险和群聊适配性，但不再单独决定最终排名。 |
| `like_target` / `comment_target` / `collect_target` / `share_target` | `100000` / `10000` / `10000` / `10000` | 对应互动项达到该数值时获得满分；使用对数曲线，较低互动也有梯度，不会让少量爆款垄断候选。 |
| `recency_window_days` | `30` | 当天发布为时效满分，超过该天数时效项为 `0`；它只影响分数，不等同于“候选最远天数”的硬过滤。 |

权重不要求手动相加为 `1`，插件会自动按总和归一化；但全部设为 `0` 会明确报错。评分明细会保存 AI 分、数据分、各项子分和实际贡献，便于审计。

### 07 · `sharing` — 主动分享与聊天流独立规则
...
| `minimum_share_score` | `0.60` | 候选进入自动分享队列的最低综合分。内容太杂可提高到 `0.80`～`0.85`，想更严格可继续上调。 |
| `candidate_selection_mode` | `最高分优先` | **候选发送顺序**。下拉可选：`随机发送`（从全部达标候选随机挑一条）、`最高分优先`（推荐默认，优先分享综合分更高的内容）、`最早收录优先`（按进入候选库的时间先后发送）。三种方式都会先遵守最低质量分、目标标签、时段、静默、冷却和每日上限。 |
...
| `douyin_video_forward_enabled` | `true` | 是否允许下载抖音短视频并直接发送到 QQ 群。私聊、未安装所选适配器、下载失败或视频超过 QQ 的时长/体积限制时会跳过该候选；不会用原链接冒充一次媒体分享。 |
...
| `manual_douyin_search_timeout_seconds` | `300` | `/抖音` 整次综合页搜索的最长时长，单位秒。达到目标会立即停止；时间用尽或页面触底后，按互动量与发布时间计算数据分并排序。 |
