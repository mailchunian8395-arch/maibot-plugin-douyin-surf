...
> [!NOTE]
> **v1.2.4 更新重点：**QQ 原生视频发送严格按 16 MB 上限预筛，不会再把 16 MB 以上的视频下载完成后才被适配器拒绝、继而只发原链接。抖音视频下载遇到 `HTTP 403` 或 `Fresh cookies needed` 时，每次明确错误都会重新前置专用浏览器，等待使用者完成登录或人机验证。
...
| `config_version` | `1.2.4` | 配置结构版本。不要手动回退或随意改写；插件更新时会用它判断配置是否需要重建。 |
...
| `douyin_video_max_bytes` | `16000000` | 下载后允许发送的最大文件字节数。NapCat 与 SnowLuma 的 QQ 群原生视频帧最高约 16 MB；设得更大也会自动按 `16000000` 执行，避免下载完成后才被适配器拒绝。 |
