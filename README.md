...
> [!NOTE]
> **v1.2.5 更新重点：**触发抖音登录或安全验证后，所有自动分享、补货、搜索和深读都会暂停，不会再由无头任务关闭刚打开的人工验证窗口；检测到窗口已丢失会重新打开并前置。QQ 原生视频仍严格按 16 MB 上限预筛。
...
| `config_version` | `1.2.5` | 配置结构版本。不要手动回退或随意改写；插件更新时会用它判断配置是否需要重建。 |
