...
| 🔎 `/抖音` 搜索 | 在抖音综合页搜索关键词并尝试发送最佳结果。 | 点赞阈值、搜索下拉次数、浏览器登录态。 |
...
配置分为十部分：`plugin`、`identity`、`surf`、`direct_text_model`、`direct_vision_model`、`candidate_filter`、`sharing`、`browser`、`video`、`retention`。下面的“默认值”以配置模型为准；已有安装如果使用不同值，以配置页当前显示的值为准。
...
| `douyin_video_forward_enabled` | `true` | 是否允许下载抖音短视频并直接发送到 QQ 群。私聊、未安装所选适配器或发送失败时自动只发说明与原链接。 |
| `video_sender_adapter` | `自动识别` | QQ 原生视频发送适配器。可选 `自动识别`（先 NapCat、后 SnowLuma）、`NapCat`、`SnowLuma`；按你实际安装的 QQ 适配器选择即可。 |
| `douyin_video_max_bytes` | `25000000` | 下载后允许发送的最大文件字节数。平台限制更严时调小。 |
