...
> [!NOTE]
> **v1.2.2 更新重点：**抖音视频下载遇到 `HTTP 403` 或 `Fresh cookies needed` 时，不会再只发送原链接并误记为已分享；插件会打开并前置专用浏览器，请使用者完成登录或人机验证后保留候选等待重试。手动 `/抖音` 仍支持可选模型短评与代表画面视觉核验；自动推荐流会优先收集候选、再分批筛选。
...
| `config_version` | `1.2.2` | 配置结构版本。不要手动回退或随意改写；插件更新时会用它判断配置是否需要重建。 |
